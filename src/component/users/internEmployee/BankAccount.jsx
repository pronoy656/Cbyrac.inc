...
//           accountFile:
//             files.length > 0
//               ? `/directDepositImage/${files[0].name}`
//               : undefined,
//           signature: allData.employeeSignature
//             ? `/employeeSignature2/${allData.employeeSignature.name}`
//             : undefined,
  ...
  const handleNext = async () => {
    ...
    if (result) {
      ...
      setFormData((prev) => ({
        ...prev,
        bankForm: {
          name: `${allData.name || ""} ${allData.lastName || ""}`.trim(),
          ssn: allData.ssn,
          checkingAccount: {
            bankName: allData.bankName,
            state: allData.state,
          ...
          savingsAccount:
            depositType === "partial"
              ? {
                  ...
                  transitNo: allData.savingsTransitNo,
                  accountNo: allData.savingsAccountNo,
                  depositPercentage: 100 - Number(selectedPercentage),
                  accountType: "Savings",
                }
              : {},
          documents: files,
          employeeSignature: allData.employeeSignature,
          signatureDate: allData.signDate,
        },
      }));
      nextStep();
        ...
        >
          ...
          <div className="mb-6">
            ...
            >
              ...
              <button
                type="button"
                className="px-6 py-2 rounded-md font-medium bg-[#8D6851] hover:bg-amber-800 text-white"
                onClick={() => document.getElementById("file-input")?.click()}
              >
                Choose File
              </button>
